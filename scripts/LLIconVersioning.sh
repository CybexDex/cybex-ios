...
arr=(${IDENTIFIER//./ }) 
APP_BUILD_NUM="${arr[${#arr[@]}-2]}.${arr[${#arr[@]}-1]}"

#APP_BUILD_NUM=$(/usr/libexec/PlistBuddy -c 'Print CFBundleIdentifier' "${INFOPLIST_FILE}")
#
...
if [ "$NEED_CREATE_NEW_ICONS" = "T" ];then
    ...
    APPICON_SET_PATH=`find $SRCROOT"/${ProjectNAME}" -name "${APPICON_NAME}.appiconset"`
