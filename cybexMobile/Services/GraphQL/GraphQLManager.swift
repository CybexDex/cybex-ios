...
class GraphQLManager {
    ...
    func verifyAddress(assetName: String, address: String) -> PromiseKit.Promise<WithdrawAddressInfo?> {
        ...
        _ = apollo.watch(query: VerifyAddressQuery(asset: assetName, address: address), cachePolicy: .fetchIgnoringCacheData, queue: Await.Queue.await) { (result, error) in
            if let _ = error {
                seal.fulfill(nil)
                return
            }
    ...
    func getWithdrawInfo(assetName: String) -> PromiseKit.Promise<WithdrawinfoObject?> {
        ...
        _ = apollo.watch(query: GetWithdrawInfoQuery(type: assetName), cachePolicy: .fetchIgnoringCacheData, queue: Await.Queue.await) { (result, error) in
            if let _ = error {
                seal.fulfill(nil)
                return
            }
    ...
    func getDepositAddress(accountName: String, assetName: String) -> PromiseKit.Promise<AccountAddressRecord?> {
        ...
        _ = apollo.watch(query: GetDepositAddressQuery(accountName: accountName, asset: assetName)) { (result, error) in
            if let _ = error {
                seal.fulfill(nil)
                return
            }
    ...
    func updateDepositAddress(accountName: String, assetName: String) -> PromiseKit.Promise<AccountAddressRecord?> {
        ...
        _ = apollo.perform(mutation: NewDepositAddressMutation(accountName: accountName, asset: assetName)) { (result, error) in
            if let _ = error {
                seal.fulfill(nil)
                return
            }
