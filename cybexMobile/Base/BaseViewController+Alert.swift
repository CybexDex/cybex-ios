...
extension UIViewController: ShowManagerDelegate {
    ...
    func showPureContentConfirm(_ title: String = R.string.localizable.tip_title.key.localized(), ensureButtonLocali: String = R.string.localizable.alert_ensure.key, content: String = "openedorder_ensure_message") {
        if ShowToastManager.shared.showView != nil {
            ShowToastManager.shared.hide(0)
        }
        SwifterSwift.delay(milliseconds: 100) {
            ...
            ShowToastManager.shared.setUp(title: title, contentView: subView, ensureButtonLocali: ensureButtonLocali, animationType: .smallBig, middleType: .normal)
            ShowToastManager.shared.showAnimationInView(self.view)
            ShowToastManager.shared.delegate = self
        }
    }
