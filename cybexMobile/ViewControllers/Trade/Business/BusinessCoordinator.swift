...
extension BusinessCoordinator: BusinessStateManagerProtocol {
    ...
    func postLimitOrder(_ pair: Pair, isBuy: Bool, callback: @escaping (_ success: Bool) -> Void) {
        guard let baseInfo = app_data.assetInfo[pair.base],
            let quoteInfo = app_data.assetInfo[pair.quote],
            let feeInfo = app_data.assetInfo[self.state.property.feeID.value],
