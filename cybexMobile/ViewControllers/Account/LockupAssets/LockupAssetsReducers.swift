...
func lockupAssteToLockUpAssetsDate(datas : [LockUpAssetsMData]) -> LockUpAssetsVMData{
  ...
  for data in datas{
    ...
    if result.eth != ""{
      ...
      count = (Double(amount)! / pow(10,Double((assetsInfo?.precision)!))).string(digits: app_data.assetInfo[quote]!.precision)
      price = "Ŵ" + (Double(count)!*Double(result.eth)! * value).string(digits: 2)
    }
