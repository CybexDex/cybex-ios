      ...
      if let order = data as? LimitOrder {
        ...
        if order.isBuy {
          ...
          if let quote_info = app_data.assetInfo[order.sellPrice.quote.assetID] ,let base_info = app_data.assetInfo[order.sellPrice.base.assetID]{
            ...
            self.amount.text = quoteAmount.doubleValue.string(digits: quote_info.precision,roundingMode:.down) + " " + quote_info.symbol.filterJade
            if self.basePriceView.isHidden == false{
              self.price.text =  baseAmount.doubleValue.string(digits: base_info.precision,roundingMode:.down) + " " + base_info.symbol.filterJade
               self.basePrice.text = base_price.doubleValue.string(digits: base_info.precision,roundingMode:.down) + " " + base_info.symbol.filterJade
            }else{
              self.price.text = base_price.doubleValue.string(digits: base_info.precision,roundingMode:.down) + " " + base_info.symbol.filterJade
            }
          }
        }
        else {
          ...
          if let quote_info = app_data.assetInfo[order.sellPrice.base.assetID],let base_info = app_data.assetInfo[order.sellPrice.quote.assetID]{
            ...
            if self.basePriceView.isHidden == false{
              self.price.text = baseAmount.doubleValue.string(digits:  base_info.precision,roundingMode:.down) + " " + base_info.symbol.filterJade
              self.basePrice.text = base_price.doubleValue.string(digits: quote_info.precision,roundingMode:.down) + " " + base_info.symbol.filterJade
