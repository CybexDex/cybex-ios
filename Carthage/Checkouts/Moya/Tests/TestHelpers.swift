...
enum HTTPBin: TargetType {
    ...
    var task: Task {
        ...
        case let .uploadMultipart(data, urlParameters), let .validatedUploadMultipart(data, urlParameters, _):
    ...
    var validationType: ValidationType {
        switch self {
        case let .validatedUploadMultipart(_, _, codes):
...
extension GitHubUserContent: TargetType {
    ...
    public var path: String {
        switch self {
        case let .downloadMoyaWebContent(contentPath), let .requestMoyaWebContent(contentPath):
...
extension ImageType {
    ...
    #elseif os(macOS)
        func asJPEGRepresentation(_: CGFloat) -> Data? {
            var imageRect = CGRect(x: 0, y: 0, width: size.width, height: size.height)
            let imageRep = NSBitmapImageRep(cgImage: cgImage(forProposedRect: &imageRect, context: nil, hints: nil)!)
