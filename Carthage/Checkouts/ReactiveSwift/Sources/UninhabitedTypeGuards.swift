...
extension SignalProducer {
    ...
    @discardableResult
    public func startWithValues(_: @escaping (Value) -> Void) -> Disposable { observingUninhabitedTypeError() }
}

extension Signal {
    @available(*, unavailable, message: "Transform the error to `NoError` beforehand, or use `observeResult` instead")
    @discardableResult
    public func observeValues(_: @escaping (Value) -> Void) -> Disposable? { observingUninhabitedTypeError() }
}
...
extension SignalProducer where Value == Never {
    ...
    public func startWithResult(_: @escaping (Result<Value, Error>) -> Void) -> Disposable { observingUninhabitedTypeError() }
}
...
extension SignalProducer where Error == NoError {
    ...
    public func startWithFailed(_: @escaping (Error) -> Void) -> Disposable { observingUninhabitedTypeError() }
}

extension Signal where Value == Never {
    @discardableResult
    @available(*, deprecated, message: "`Result.success` is never delivered - value type `Never` is uninstantiable (Use at runtime would trap)")
    public func observeResult(_: @escaping (Result<Value, Error>) -> Void) -> Disposable? { observingUninhabitedTypeError() }
}
...
extension Signal where Error == NoError {
    ...
    public func observeFailed(_: @escaping (Error) -> Void) -> Disposable? { observingUninhabitedTypeError() }
}
...
extension SignalProducer where Value == Never {
    ...
    public func flatMap<Inner: SignalProducerConvertible>(_: FlattenStrategy, _: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Error> where Inner.Error == Error { observingUninhabitedTypeError() }

    @discardableResult
    @available(*, deprecated, message: "Use `promoteValue` instead - value type `Never` is uninstantiable (Use at runtime would trap)")
    public func flatMap<Inner: SignalProducerConvertible>(_: FlattenStrategy, _: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Error> where Inner.Error == NoError { observingUninhabitedTypeError() }
}
...
extension SignalProducer where Value == Never, Error == NoError {
    ...
    public func flatMap<Inner: SignalProducerConvertible>(_: FlattenStrategy, _: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Inner.Error> { observingUninhabitedTypeError() }

    @discardableResult
    @available(*, deprecated, message: "Use `promoteValue` instead - value type `Never` and error type `NoError` are uninstantiable (Use at runtime would trap)")
    public func flatMap<Inner: SignalProducerConvertible>(_: FlattenStrategy, _: @escaping (Value) -> Inner) -> SignalProducer<Inner.Value, Inner.Error> where Inner.Error == Error { observingUninhabitedTypeError() }
}

extension SignalProducer where Error == NoError {
    @discardableResult
    @available(*, deprecated, message: "Use `promoteError` instead - error type `NoError` is uninstantiable (Use at runtime would trap)")
    public func flatMapError<NewError>(_: @escaping (Error) -> SignalProducer<Value, NewError>) -> SignalProducer<Value, NewError> { observingUninhabitedTypeError() }
}

extension Signal where Value == Never {
    @discardableResult
    @available(*, deprecated, message: "Use `promoteValue` instead - value type `Never` is uninstantiable (Use at runtime would trap)")
    public func flatMap<Inner: SignalProducerConvertible>(_: FlattenStrategy, _: @escaping (Value) -> Inner) -> Signal<Inner.Value, Error> where Inner.Error == Error { observingUninhabitedTypeError() }

    @discardableResult
    @available(*, deprecated, message: "Use `promoteValue` instead - value type `Never` is uninstantiable (Use at runtime would trap)")
    public func flatMap<Inner: SignalProducerConvertible>(_: FlattenStrategy, _: @escaping (Value) -> Inner) -> Signal<Inner.Value, Error> where Inner.Error == NoError { observingUninhabitedTypeError() }
}

extension Signal where Value == Never, Error == NoError {
    @discardableResult
    @available(*, deprecated, message: "Use `promoteValue` instead - value type `Never` and error type `NoError` are uninstantiable (Use at runtime would trap)")
    public func flatMap<Inner: SignalProducerConvertible>(_: FlattenStrategy, _: @escaping (Value) -> Inner) -> Signal<Inner.Value, Inner.Error> { observingUninhabitedTypeError() }

    @discardableResult
    @available(*, deprecated, message: "Use `promoteValue` instead - value type `Never` and error type `NoError` are uninstantiable (Use at runtime would trap)")
    public func flatMap<Inner: SignalProducerConvertible>(_: FlattenStrategy, _: @escaping (Value) -> Inner) -> Signal<Inner.Value, Inner.Error> where Inner.Error == Error { observingUninhabitedTypeError() }
}

extension Signal where Error == NoError {
    @discardableResult
    @available(*, deprecated, message: "Use `promoteError` instead - error type `NoError` is uninstantiable (Use at runtime would trap)")
    public func flatMapError<NewError>(_: @escaping (Error) -> SignalProducer<Value, NewError>) -> Signal<Value, NewError> { observingUninhabitedTypeError() }
}
