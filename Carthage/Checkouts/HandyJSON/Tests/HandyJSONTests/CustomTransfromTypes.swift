...
struct CustomMappingStruct: HandyJSON {
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            id <-- TransformOf<String, String>(fromJSON: { (rawValue) -> String? in
                if let str = rawValue {
                    return "json_" + str
                }
                return nil
        ...
        mapper <<<
            height <-- ("json_height", TransformOf<Int, String>(fromJSON: { (rawValue) -> Int? in
...
class CustomMappingClass: HandyJSON {
    ...
    func mapping(mapper: HelpingMapper) {
        ...
        // specify converting method
        mapper <<<
            id <-- TransformOf<String, String>(fromJSON: { (rawStr) -> String? in
                if let _str = rawStr {
                    return "json_" + _str
                }
                return nil
        ...
        // specify both
        mapper <<<
            height <-- ("json_height", TransformOf<Int, Int>(fromJSON: { (rawInt) -> Int? in
                if let _int = rawInt {
                    return _int / 100
                }
                return nil
...
class KeyArrayMappingClass: HandyJSON {
    ...
    func mapping(mapper: HelpingMapper) {
        mapper <<<
            var1 <-- ["var1_v1", "var1_v2", "var1_v3"]

        mapper <<<
            var2 <-- (["var2_v1", "var2_v2", "var2_v3"], TransformOf<String, String>(fromJSON: { (rawStr) -> String? in
                rawStr
                }, toJSON: { (srcStr) -> String? in
                    srcStr
            }))
